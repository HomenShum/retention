...
|--------|---------|-----------|-----|--------|-----------|--------|
| **Workflow** | schemas.py:58 | ta.pipeline.run | /api/workflow_registry | ❌ | TrajectoriesPage | PARTIAL — not synced to Convex |
| **Run** | run_session.py:49 | ta.pipeline.* | /api/benchmarks | benchmarkRuns | BenchmarkReportPage | ✅ FULL |
...
| **Artifact** | evidence_schema.py:54 | ta.pipeline.screenshot | /api/artifacts | chefArtifacts | ReportPage | ✅ FULL |
| **FailureBundle** | evidence_schema.py:206 | ta.pipeline.failure_bundle | /api/failures | ❌ derived | ReportPage, Slack | ✅ FULL |
| **MemoryObject** | exploration_memory.py | ta.memory.* (5 tools) | /api/memory | institutionalMemory, teamMemory | MemoryDashboard | PARTIAL — siloed |
...
```
ta.qa_check(url)          — instant QA verdict
ta.sitemap(url)           — interactive site map with drill-down
ta.ux_audit(url)          — 21-rule UX audit
ta.crawl.url(url)         — full crawl with screenshots + findings
ta.diff_crawl(url)        — before/after delta
...
ta.savings.compare        — show token/time savings
ta.team.invite            — generate Slack onboarding message
ta.team.status            — team membership + dashboard URL
ta.onboard.status         — prerequisites check
ta.system_check           — health + version + update notification
```

### Target tools (from architecture vision)
```
ta.start_workflow(id, target, mode)    — start with trajectory awareness
...
ta.memory.lookup(entity)               — contextual memory retrieval
ta.memory.handoff(run_id)              — session handoff summary
ta.memory.rollup(period, family)       — aggregated rollup
